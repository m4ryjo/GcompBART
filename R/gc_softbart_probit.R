#' @title SoftBART Probit Model
#' @description Fits a Bayesian Additive Regression Trees (SoftBART) model for binary outcomes using a probit link.
#' @param X A numeric matrix of predictors for training.
#' @param Y A binary outcome vector (0/1 or factor with two levels).
#' @param X_test Optional numeric matrix of predictors for testing.
#' @param num_tree Number of trees in the ensemble (default = 20).
#' @param k Prior parameter controlling shrinkage (default = 1).
...
#' \describe{
#'   \item{mu_train_mean}{Posterior mean predictions for training data (latent scale).}
#'   \item{mu_test_mean}{Posterior mean predictions for test data (latent scale, if provided).}
#'   \item{offset}{Probit offset based on observed proportion.}
...
#' fit <- gc_softbart_probit(X, Y, num_tree = 50)
#' str(fit)
#' }
#' @export
