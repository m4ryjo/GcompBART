...
Forest::~Forest() {
  for(arma::uword i = 0; i < trees.size(); i++) {
    delete trees[i];
  }
}
...
// Updated for GcompBart:
// Added flags: update_tvp, update_eta, update_phi

Opts InitOpts(int num_burn, int num_thin, int num_save, int num_print,
              bool update_sigma_mu, bool update_s, bool update_alpha, bool update_tvp,
              bool update_eta, bool update_phi, bool update_beta, bool update_gamma, bool update_tau,
  ...
  out.update_tvp = update_tvp;
  out.update_eta = update_eta;
  out.update_phi = update_phi;
...
// Updated for GcompBart:
// Added support for time-varying groups (tgroup, tgroup_size)
// Added new hyperparameters: eta, phi, alpha_vec
Hypers InitHypers(const mat& X, const uvec& group, const uvec& tgroup, const uvec& tgroup_size,
                  double sigma_hat,
                  double alpha, double eta, const arma::vec& phi, const arma::vec& alpha_vec,
                  double beta,
  ...
  out.num_tgroups = tgroup.max();
  out.s = ones<vec>(out.num_groups) / ((double)(out.num_groups));
  out.logs = log(out.s);

  out.idx_t = find(out.tgroup == out.tgroup.max());
  out.idx_tv = find(out.tgroup < out.tgroup.max());
  out.pt = 1/((double)(out.idx_t.size())) * ones<vec>(out.idx_t.size());
  out.logpt = log(out.pt);
  out.w = (double)out.idx_t.size()/(double)(out.idx_tv.size()+out.idx_t.size());
  out.logw = log(out.w);
  out.v = ones<vec>(out.tgroup.size()) / ((double)(out.tgroup.size()));
  out.logv = log(out.v);
  out.u = 1/((double)(out.idx_tv.size())) * arma::ones<arma::vec>(out.idx_tv.size());
  out.logu = log(out.u);

  out.alpha_shape_1_vec = ones<vec>(out.tgroup.max());
  for(arma::uword i = 0; i < out.tgroup.max(); i++) {
    out.alpha_shape_1_vec(i) = out.alpha_shape_1 + (double)i*(1-out.alpha_shape_1)/(double)out.tgroup.max();
  }
...
void IterateGibbsWithS(std::vector<Node*>& forest, arma::vec& Y_hat, const arma::vec& weights,
  ...
  if(opts.update_tvp) {
     if(hypers.tgroup.max()==1){
         UpdateS(forest, hypers);
     } else {
         UpdateTVP(forest, hypers);
         if(opts.update_eta) hypers.UpdateEta();
         if(opts.update_phi) hypers.UpdatePhi();
         if(opts.update_alpha_vec) hypers.UpdateAlpha_vec();
     }
  }
...
std::vector<double> get_perturb_limits(Node* branch) {
  ...
  for(arma::uword i = 0; i < left_branches.size(); i++) {
    if(left_branches[i]->var == branch->var) {
      if(left_branches[i]->val > min)
        min = left_branches[i]->val;
    }
  }
  for(arma::uword i = 0; i < right_branches.size(); i++) {
...
void UpdateS(std::vector<Node*>& forest, Hypers& hypers) {
  ...
  for(arma::uword i = 0; i < shape_up.size(); i++) {
...
// This function updates the time-varying prior (TVP) for time point grouped
// predictors in a longitudinal model. It adjusts hierarchical probabilities for
// variable selection based on the current forest structure and hyperparameters.
// Steps:
// 1. Compute variable counts from the forest.
// 2. Update w: weight for time-specific vs non-time-specific groups.
// 3. Update pt: probabilities for time-specific variables.
// 4. Update u: group-level probabilities for time-varying components.
// 5. Update v: variable-level probabilities within each time group.
// Combines these to update hypers.s for dynamic longitudinal modeling.

void UpdateTVP(std::vector<Node*>& forest, Hypers& hypers) {

  // Get shape vector
  uvec gvc = get_var_counts(forest, hypers);
  vec shape_u(hypers.tgroup.max() - 1);
  vec shape_w = 1/(double)2 * ones<vec>(2);

  for(arma::uword k = 0; k < 2; k++) {
    if(k < 1) {
      double gvc_t = sum(gvc.elem(hypers.idx_t));

      // Update w
      shape_w(0) = shape_w(0) + gvc_t;
      shape_w(1) = shape_w(1) + (sum(gvc) - gvc_t); ///(double)hypers.tgroup.max();

      vec lgsw = zeros<vec>(2);
      for(arma::uword i = 0; i < 2; i++) {
        lgsw(i) = rlgam(shape_w(i));
      }

      // Normalize s on the log scale, then exponentiate
      hypers.logw = lgsw - log_sum_exp(lgsw);
      hypers.w = exp(hypers.logw(0));

      // Update Pt
      vec shape_pt = hypers.eta/(double)hypers.idx_t.size() * ones<vec>(hypers.idx_t.size());
      shape_pt = shape_pt + gvc.elem(hypers.idx_t);
      vec lgspt = zeros<vec>(shape_pt.size());
      for(arma::uword i = 0; i < shape_pt.size(); i++) {
        lgspt(i) = rlgam(shape_pt(i));
      }
      // Normalize s on the log scale, then exponentiate
      hypers.logpt = lgspt - log_sum_exp(lgspt);
      hypers.pt = exp(hypers.logpt);
      hypers.s.elem(hypers.idx_t) = hypers.pt * hypers.w;

    } else {
      // Update U (tv-part)
      shape_u = hypers.alpha_vec / ((double)hypers.alpha_vec.size());
      for(arma::uword j = 0; j < hypers.tgroup.max() - 1; j++) {
        int tmp = j + 1;
        uvec idx = find(hypers.tgroup == tmp);
        shape_u(j) = shape_u(j) + sum(gvc.elem(idx));
      }

      vec lgsu = zeros<vec>(shape_u.size());
      for(arma::uword i = 0; i < shape_u.size(); i++) {
        lgsu(i) = rlgam(shape_u(i));
      }

      hypers.logu = lgsu - log_sum_exp(lgsu);
      hypers.u = exp(hypers.logu);
    ...
    for(arma::uword j = 0; j < hypers.tgroup.max() - 1; j++) {
      int tmp = j + 1;
      uvec idx = find(hypers.tgroup == tmp);
      vec shape_v =  hypers.phi(j)/(double)idx.size() * ones<vec>(idx.size());

      shape_v = shape_v + gvc.elem(idx);
      vec lgsv = zeros<vec>(shape_v.size());

      for(arma::uword i = 0; i < shape_v.size(); i++) {
        lgsv(i) = rlgam(shape_v(i));
      }
      // Normalize s on the log scale, then exponentiate
      hypers.logv.elem(idx) = lgsv - log_sum_exp(lgsv);
      hypers.v.elem(idx) = exp(hypers.logv.elem(idx));

      hypers.s.elem(idx) = hypers.v.elem(idx) * hypers.u(j) * (1 - hypers.w);
    }
  }
}
...
arma::vec rdirichlet(const arma::vec& shape) {
  ...
  for(arma::uword i = 0; i < shape.size(); i++) {
    do {
...
void Hypers::UpdateAlpha() {
  ...
  for(arma::uword i = 0; i < rho_propose.size(); i++) {
...
// Extension for GcompBart:
// Updates eta hyperparameter for longitudinal prior.
// Steps:
// 1. Compute log-likelihoods for proposed rho values.
// 2. Normalize and convert to probabilities.
// 3. Sample new rho and transform to eta using rho_to_alpha.
// Ensures eta reflects posterior distribution for dynamic modeling
void Hypers::UpdateEta() {
  arma::vec logliks = zeros<vec>(rho_propose.size());

  rho_loglik loglik;
  loglik.mean_log_s = mean(logpt);
  loglik.p = (double)pt.size();
  loglik.alpha_scale = alpha_scale;
  loglik.alpha_shape_1 = alpha_shape_1;
  loglik.alpha_shape_2 = alpha_shape_2;
  ...
  logliks = exp(logliks - log_sum_exp(logliks));
  double rho_up = rho_propose(sample_class(logliks));
  eta = rho_to_alpha(rho_up, alpha_scale);

}

// Extension for GcompBart:
// Updates phi hyperparameter for each time group.
// Steps:
// 1. Loop over time groups and compute mean log-probabilities excluding current group.
// 2. Compute log-likelihoods for proposed rho values.
// 3. Normalize and convert to probabilities.
// 4. Sample new rho and transform to phi using rho_to_alpha.
void Hypers::UpdatePhi() {
  arma::vec logliks = zeros<vec>(rho_propose.size());

  for(int t = 0; t < num_tgroups - 1; t++) {
    uvec idx = find(tgroup != t);
    vec logv_t = logv.elem(idx);

    rho_loglik loglik;
    loglik.mean_log_s = mean(logv_t);
    loglik.p = (double)idx.size();
    loglik.alpha_scale = alpha_scale;
    loglik.alpha_shape_1 = alpha_shape_1;
    loglik.alpha_shape_2 = alpha_shape_2;
    ...
    logliks = exp(logliks - log_sum_exp(logliks));
    double rho_up = rho_propose(sample_class(logliks));
    phi(t) = rho_to_alpha(rho_up, alpha_scale);
  }
}

// Extension for GcompBart:
// Updates alpha_vec hyperparameter for time-varying groups.
// Steps:
// 1. Loop over subgroups and compute summaries excluding current subgroup.
// 2. Calculate terms for rho_loglik_tv likelihood.
// 3. Compute log-likelihoods for proposed rho values.
// 4. Normalize and convert to probabilities.
// 5. Sample new rho and transform to alpha using rho_to_alpha.
void Hypers::UpdateAlpha_vec() {
  arma::vec logliks(rho_propose.size());
  int sub_size = tgroup.max() - 1;
  arma::uvec v = linspace<uvec>(1, sub_size, sub_size);
  arma::uvec idx(sub_size);
  arma::vec alpha_not_t(sub_size);
  arma::vec logu_not_t(sub_size);
  double sum_lng_alpha_by_p_not_t = 0.;
  double p = sub_size;

  for(int t = 0; t < sub_size; t++) {
    idx = find(v != t + 1);
    alpha_not_t = alpha_vec.elem(idx);
    logu_not_t = logu.elem(idx);
    double logu_t = logu(t);
    double sum_alpha_by_p_not_t = sum(alpha_not_t) / p;
//    double sum_alpha_by_p_logs_not_t = sum(alpha_not_t % logu_not_t);

    for(arma::uword j = 0; j < idx.size(); j++){
      sum_lng_alpha_by_p_not_t = sum_lng_alpha_by_p_not_t + Rf_lgammafn(alpha_not_t(j) / p);
    }

    rho_loglik_tv loglik;
    loglik.logu_t = logu_t;
    loglik.p = p;
    loglik.sum_lng_a_by_p_not_t = sum_lng_alpha_by_p_not_t;
    loglik.sum_a_by_p_not_t = sum_alpha_by_p_not_t;
    loglik.sum_a_by_p_logs_not_t = sum_lng_alpha_by_p_not_t;
    loglik.alpha_scale = p;
    loglik.alpha_shape_1 = alpha_shape_1_vec(t);
    loglik.alpha_shape_2 = alpha_shape_2;
    ...
    logliks = exp(logliks - log_sum_exp(logliks));
    double rho_up = rho_propose(sample_class(logliks));
    alpha_vec(t) = rho_to_alpha(rho_up, alpha_scale);
  }
}
...
double forest_loglik(std::vector<Node*>& forest, double gamma, double beta) {
  double out = 0.0;
  for(arma::uword t = 0; t < forest.size(); t++) {
...
std::vector<Node*> copy_forest(std::vector<Node*> forest, Hypers& hypers) {
  ...
  for(arma::uword i = 0; i < forest.size(); i++) {
...
// Updated for GcompBart:
// [[Rcpp::export]] exposes this function to R.
// Steps:
// 1. Collect all input data and hyperparameters, including new longitudinal components (tgroup, tgroup_size, eta, phi, alpha_vec).
// 2. Initialize options using InitOpts (includes new update flags for TVP, eta, phi).
// 3. Initialize hyperparameters using InitHypers (extended for time-varying and hierarchical priors).
// 4. Call do_soft_bart() to run the Bayesian tree ensemble with updated longitudinal structure.
// Returns: A List containing posterior samples and predictions.
List SoftBart(const arma::mat& X, const arma::vec& Y, const arma::mat& X_test,
              const arma::uvec& group, const arma::uvec& tgroup, const arma::uvec& tgroup_size,
              double alpha, double eta, const arma::vec& phi, const arma::vec& alpha_vec,
              double beta, double gamma, double sigma,
              double shape, double width, int num_tree,
              double sigma_hat, double k, double alpha_scale,
              double alpha_shape_1, double alpha_shape_2,
              double tau_rate,
              ...
              bool update_s, bool update_alpha, bool update_tvp, bool update_eta, bool update_phi,
  ...
  Opts opts = InitOpts(num_burn, num_thin, num_save, num_print, update_sigma_mu,
                       update_s, update_alpha, update_tvp, update_eta, update_phi, update_beta, update_gamma,
                       update_tau, update_tau_mean, update_num_tree,
                       update_sigma);

  Hypers hypers = InitHypers(X, group, tgroup, tgroup_size, sigma_hat, alpha, eta, phi, alpha_vec,
                             beta, gamma, k, width,
...
Hypers::Hypers(Rcpp::List hypers) {
  alpha = hypers["alpha"];
  eta = hypers["eta"];
  phi = as<arma::vec>(hypers["phi"]);
  alpha_vec = as<arma::vec>(hypers["alpha_vec"]);
  beta = hypers["beta"];
  gamma = hypers["gamma"];
  sigma = hypers["sigma"];
  ...
  tgroup = as<arma::uvec>(hypers["tgroup"]);
  tgroup_size = as<arma::uvec>(hypers["tgroup_size"]);
  num_tgroups = tgroup.max();
  ...
  idx_t = find(tgroup == tgroup.max());
  idx_tv = find(tgroup < tgroup.max());
  pt = 1/((double)(idx_t.size())) * arma::ones<arma::vec>(idx_t.size());
  logpt = log(pt);
  //w = 1/((double)(2));
  w = (double)idx_t.size()/(double)(idx_tv.size()+idx_t.size());
  logw = log(w);
  v = 1/((double)tgroup.size()) * arma::ones<arma::vec>(tgroup.size());
  logv = log(v);
  u = 1/((double)(idx_tv.size())) * arma::ones<arma::vec>(idx_tv.size());
  logu = log(u);

  alpha_shape_1_vec = ones<vec>(tgroup.max());
  for(arma::uword i = 0; i < tgroup.max(); i++) {
    alpha_shape_1_vec(i) = alpha_shape_1 + (double)i*(1 - alpha_shape_1)/(double)tgroup.max();
  }
...
std::vector<Node*> AddTree(std::vector<Node*>& forest,
  ...
  for(arma::uword i = 0; i < leafs.size(); i++) {
...
void RenormAddTree(std::vector<Node*>& forest,
  ...
  if(RESCALE) {
    ...
    // Scale the leaves
    for(arma::uword i = 0; i < new_forest.size(); i++) {
      std::vector<Node*> leafs = leaves(new_forest[i]);
      for(arma::uword j = 0; j < leafs.size(); j++) {
...
arma::mat Forest::do_gibbs(const arma::mat& X, const arma::vec& Y,
  ...
  for(int i = 0; i < num_iter; i++) {
    ...
    } else if(opts.update_tvp && (num_gibbs > num_warmup)) {
      IterateGibbsWithS(trees, Y_hat, weights, hypers, X, Y, opts);
    } else {
...
arma::vec Forest::predict_iteration(const arma::mat& X, std::size_t r_iter) {
  std::size_t cpp_iter = r_iter - 1;
  if (r_iter > saved_forests.size())
    stop("Specified iteration exceeds number of saved trees");
  return predict(saved_forests[cpp_iter], X, hypers);
}
